...
erode = imerode(grayImage, disk);
dilate = imdilate(grayImage, disk);
grayImage = imsubtract(dilate, erode);
grayImage = imclearborder(grayImage, 26); % Use 1, 4, 6, 8, 18, 26
threshold = graythresh(grayImage);
% Transform the image to binary
binImage = ~imbinarize(grayImage, threshold);
% Remove objects smaller than 30 pixels);
binImage = bwareaopen(binImage, 40);

% Remove small objects from binary image
modImage = bwareaopen(binImage, 3750);
I = binImage - modImage;
I = bwareaopen(I, 100);
